...
export const HeroSection = () => {
  ...
  return (
    <section className="grain premium-card hero-surface relative overflow-hidden rounded-3xl border border-gold/30 px-6 py-20 text-center md:px-12 md:py-24">
      ...
      <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="font-display text-5xl text-ivory md:text-7xl">
        {settings.heroTitle}
      </motion.h1>
      <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2, duration: 0.5 }} className="mx-auto mt-4 max-w-2xl text-lg text-ivory/85 md:text-xl">
        {settings.heroSubtitle}
      </motion.p>
      <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.32, duration: 0.45 }} className="float-slow">
        <Link to="/rooms" className="mt-8 inline-block rounded-xl border border-gold bg-gold px-7 py-3 font-accent uppercase tracking-wider text-auburn transition hover:-translate-y-0.5 hover:brightness-105">
          Explore Rooms
        </Link>
      </motion.div>
      <motion.div
      ...
      >
        {[
          ...
          <div key={item.label} className="rounded-xl border border-gold/25 bg-black/25 px-3 py-3 backdrop-blur-sm">
            <p className="font-display text-2xl text-gold">{item.value}</p>
            <p className="text-xs uppercase tracking-[0.16em] text-ivory/75">{item.label}</p>
          </div>
        ))}
      </motion.div>
    </section>
